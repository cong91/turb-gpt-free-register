...
class RoxyRegistrationOtpTests(unittest.TestCase):
    @patch("core.registration_flow._wait_after_email_otp_submit", side_effect=["accepted"])
    @patch("core.registration_flow._click_continue")
    @patch(
        "core.registration_flow._type_otp",
        side_effect=[RuntimeError("找不到 OTP 输入框"), None],
    )
    @patch("core.registration_flow._clear_otp_inputs")
    @patch("core.registration_flow._click_resend_email_otp")
    @patch("core.registration_flow.wait_for_otp", return_value="654321")
    ...
    def test_missing_otp_input_resends_and_fetches_a_new_code(
        ...
        with patch("core.registration_flow.human_delay"), patch.object(
    ...
    @patch("core.registration_flow._wait_after_email_otp_submit", return_value="accepted")
    @patch("core.registration_flow._click_continue")
    @patch("core.registration_flow._type_otp")
    @patch("core.registration_flow._clear_otp_inputs")
    @patch("core.registration_flow._click_resend_email_otp")
    @patch(
        "core.registration_flow.wait_for_otp",
    ...
    @patch("core.registration_flow._wait_after_email_otp_submit", return_value="invalid")
    @patch("core.registration_flow._click_continue")
    @patch("core.registration_flow._type_otp")
    @patch("core.registration_flow._clear_otp_inputs")
    @patch("core.registration_flow._click_resend_email_otp")
    @patch("core.registration_flow.wait_for_otp", return_value="222222")
    @patch("core.registration_flow.time.time", side_effect=[100.0, 200.0])
    ...
    @patch("core.registration_flow._wait_after_email_otp_submit")
    @patch("core.registration_flow._click_continue")
    @patch("core.registration_flow._type_otp")
    @patch("core.registration_flow._clear_otp_inputs")
    @patch("core.registration_flow._click_resend_email_otp")
    @patch("core.registration_flow._submit_email_and_wait_next", return_value="otp")
    @patch("core.registration_flow._fill_password_page_if_present", return_value="pw")
    @patch("core.registration_flow._click_continue_with_password_link")
    @patch("core.registration_flow._reset_login_page_for_retry")
    @patch("core.registration_flow._is_chrome_error_page", return_value=True)
    @patch("core.registration_flow.wait_for_otp", return_value="222222")
    @patch("core.registration_flow.time.time", side_effect=[100.0, 200.0])
    ...
    def test_is_chrome_error_page_detects_url_and_http_500_text(self):
        ...
        with patch("core.browser_page_actions._email_otp_page_state", return_value=state):
            self.assertTrue(browser_page_actions._is_chrome_error_page(driver))
