...
from core import pay153_stripe_checkout as sc
from core.pay153_billing_address_resolver import resolve_public_address
                ...
                (
                    "pix_monthly",
                    {
                        "mandate_options": {
                            ...
                            "start_date": (local_today() + timedelta(days=3)).isoformat(),
                        }
                    },
                ),
                ("pix_pm_only", {}),
...
def confirm_provider_payment(
    ...
    *,
    ...
    if zero_due:
        ...
        if provider == "pix":
            ...
            if provider == "pix":
                ...
                mandate.setdefault("start_date", (local_today() + timedelta(days=3)).isoformat())
            else:
    ...
    option_keys = sorted(
        key for key in data
        if key.startswith(("payment_method_options[", f"payment_method_data[{provider}][mandate_options]"))
    )
    log(
    ...
    if not out.get("provider_redirect_url") and not out.get("qr_image_png") and not out.get("qr_data"):
        ...
        if sub.get("state") == "requires_approval" and approve_callback:
            if late_promo and apply_promo_callback:
                ...
                apply_promo_callback(processor)
                _promo_init, _promo_version, promo_ctx = sc.init_checkout(http, session_id, pk, profile, log)
