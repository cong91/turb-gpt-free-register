...
from core.pay153_provider_policy import (
    PROVIDER_POLICIES,
    provider_policy,
...
def _custom_confirm_and_start(
    ...
    if str(payload.get("status") or "").lower() != "requires_action" or not isinstance(action, dict):
        raise RuntimeError("Custom checkout did not return a payment action")
...
def _custom_action_material(action: dict[str, Any], provider: str) -> dict[str, Any]:
    """Normalize a custom-checkout action using the existing provider parser."""
    material = provider_checkout.extract_provider_result({"next_action": action}, provider)
    direct_url = str(action.get("url") or action.get("redirect_url") or "").strip()
    if direct_url and not material.get("provider_redirect_url"):
        material["provider_redirect_url"] = direct_url
    return material
...
def _run_custom_provider(
    ...
    result = {
        ...
        "short_link": redirect,
        "custom_payment_method_id": str(selected_method["id"]),
        ...
        "expires_at": payment_material.get("expires_at") or int(time.time()) + 1800,
    }
