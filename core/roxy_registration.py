...
def _submit_email_and_wait_next(driver, email: str | None, attempts: int = 3, email_supplier=None) -> str:
...
def _click_continue_with_password_link(driver) -> bool:
    ...
    if clicked:
        ...
        time.sleep(2.0)
        return True
    logger.info("%s 未找到 Continue with password，直接导航到密码页", _log_prefix(driver))
    try:
