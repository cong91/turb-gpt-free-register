...
- 已有账号登录和 2FA re-auth 使用独立的 `TWOFA_OTP_MAX_WAIT`，默认 90 秒，不会改变普通注册的 `OTP_MAX_WAIT`；如果邮箱投递较慢，可在「邮箱 / OTP」适当调大，避免过早重新创建 auth step 触发上游限流。
...
| `config/twofa.py` | 2FA 开关、已有账号登录/2FA re-auth OTP 等待时间 |
