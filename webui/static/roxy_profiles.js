(function () {
  ...
  const labels = {
    ...
    SNAPSHOTTING: 'Đang snapshot', ARCHIVE_COMMITTED: 'Đã archive', SOFT_DELETE_PENDING: 'Đang archive',
    TRASHED: 'Trong Trash', RESTORE_REQUIRED: 'Cần restore', OFFLINE_STAGING: 'Đang chuẩn bị local',
    OFFLINE_RUNNING: 'Local đang chạy', OFFLINE_STOPPED: 'Local đã dừng', OFFLINE_UNVERIFIED: 'Local cần kiểm tra',
  ...
  function archiveCell(profile) {
    ...
    return `<span class="roxy-profile-archive-state">${full ? 'Full v2' : 'Metadata v1'}<br><small>${full ? 'browser_state_only' : 'Cần restore Roxy trước khi mở'}</small></span>`;
  }
  function runtimeCell(profile) {
    ...
    const cdp = launch.backend ? 'CDP active' : 'CDP inactive';
    const parity = launch.fingerprint_status || 'unknown';
    return `<span class="roxy-profile-runtime"><strong>${esc(backend)}</strong><br><small>Core ${esc(core)} · ${esc(cdp)} · parity ${esc(parity)}</small></span>`;
  }
  function actions(profile) {
    ...
    return `<div class="roxy-profile-actions">
      <button type="button" class="roxy-profile-action" data-action="edit" data-id="${id}" title="Sửa Roxy profile" ${['ACTIVE_STOPPED', 'RUNNING'].includes(stateName) ? '' : 'disabled'}>✎</button>
      ...
      <button type="button" class="roxy-profile-action" data-action="export-full" data-id="${id}" title="Export full-folder v2" ${canFullExport ? '' : 'disabled'}>⇩</button>
      <button type="button" class="roxy-profile-action" data-action="local-open" data-id="${id}" title="Mở local thử nghiệm: browser_state_only" ${canLocalOpen ? '' : 'disabled'}>◫</button>
      <button type="button" class="roxy-profile-action" data-action="local-close" data-id="${id}" title="Đóng local và checkpoint" ${canLocalClose ? '' : 'disabled'}>□</button>
      <button type="button" class="roxy-profile-action" data-action="download" data-id="${id}" title="Tải archive đã mã hóa" ${profile.archive ? '' : 'disabled'}>↓</button>
      <button type="button" class="roxy-profile-action roxy-profile-action--danger" data-action="archive" data-id="${id}" title="Full export rồi soft-delete vào Trash" ${canArchive ? '' : 'disabled'}>⌫</button>
    </div>`;
  }
  ...
  function renderStatus(status) {
    ...
    if (summary) summary.textContent = `${status.managed_count || 0} managed · ${status.active_remote_count || 0} active Roxy slot`;
    if (capabilities) {
      capabilities.textContent = status.offline_open_supported ? 'Local thử nghiệm đã bật' : 'Local thử nghiệm đang tắt';
  ...
  async function importProfile(event) {
    ...
    catch (error) { setStatus(error.message || 'Import thất bại', true); }
  }
  async function runBulk(action, confirmMessage = '') {
    ...
    try {
      ...
      await refresh(false);
    } catch (error) { setStatus(error.message || 'Bulk operation failed', true); }
  }
  ...
  async function handleAction(event) {
    ...
    if (action === 'archive' && !window.confirm('Full export rồi soft-delete profile vào Roxy Trash?')) return;
    ...
    catch (error) { setStatus(error.message || 'Profile operation failed', true); }
    finally { button.disabled = false; }
  }
  ...
  $('#btnBulkArchiveRoxyProfiles')?.addEventListener('click', () => runBulk('archive', 'Full export rồi soft-delete các profile đã chọn?'));
